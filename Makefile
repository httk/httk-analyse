...
docs-lock-check: docs-clean
	@set -eu; \
	check_dir=$$(mktemp -d "$${TMPDIR:-/tmp}/httk-analyse-docs-lock-check.XXXXXX"); \
...
dist-clean:
	rm -rf build $(DIST_DIR) src/httk_analyse.egg-info
...
format:
	...
	$(PYTHON) -m ruff format src examples benchmarks tools

format-check: lint
	$(PYTHON) -m ruff format --check src examples benchmarks tools

lint:
	$(PYTHON) -m ruff check src examples benchmarks tools
	pydoclint --quiet src
